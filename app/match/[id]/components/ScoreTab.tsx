  ...
  return (
    <div role="tabpanel" id="panel-score" className="space-y-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
            ...
            {hasUnsavedScore ? (
              <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 px-3 py-2 rounded-lg">
                <span className="text-xs font-bold text-amber-600 flex-1">
                  {saved ? "✅ Marcador guardado" : "⚠️ Marcador sin guardar"}
                </span>
                <button
              ...
              <div className="flex items-center justify-center gap-2 bg-emerald-50 text-emerald-700 px-4 py-2 rounded-lg text-xs font-bold border border-emerald-200">
                ✅ Marcador guardado — listo para cerrar el partido
              </div>
            )}
          </div>
        )}
        ...
        {isClosed && (
          <div className="mt-4 bg-slate-50 text-slate-500 px-4 py-2 rounded-lg text-xs font-bold border border-slate-200 text-center">
            🔒 Partido cerrado
          </div>
        )}
      </div>
