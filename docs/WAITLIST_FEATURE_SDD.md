...
| 9 | Los jugadores en lista de espera NO pagan depósito al anotarse. Si son aceptados por un admin (`approveFromWaitlist`), tampoco se cobra depósito. `depositPaid` permanece `undefined`/`false` para jugadores promovidos desde waitlist. | `joinWaitlist()` y `approveFromWaitlist()` no tocan saldo ni `depositPaid` |
| 10 | Al cancelarse un partido (`deleteMatchWithRefunds`), todos los jugadores con uid — incluyendo los que están en lista de espera — reciben notificación in-app informando la cancelación. Los de waitlist no reciben mención de depósito porque nunca pagaron. | `deleteMatchWithRefunds` en `functions/src/payments.ts` filtra `notifiablePlayers` por `p.uid` (incluye waitlist) |
