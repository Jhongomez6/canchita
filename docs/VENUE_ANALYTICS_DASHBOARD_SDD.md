...
| RN-15 | **Detalle de inasistencias** y **detalle de reservas canceladas**: lista de las instancias del rango (fecha, hora, cancha, cliente; motivo de cancelación si se registró en el slot), ordenadas por fecha desc, con tope visible + "+N más". Es **PII** (nombres de cliente) → solo dentro del área admin, nunca en eventos de analytics. | Listas de detalle |
...
| `components/booking/ClientRankList.tsx` | **Nuevo** — ranking de clientes (ingresos / reservas / cancelaciones / inasistencias) |
| `components/booking/ReservationDetailList.tsx` | **Nuevo** — detalle de inasistencias / cancelaciones (fecha, hora, cancha, cliente, motivo) |
