...
| RN-04 | Los **ingresos** se calculan **solo desde `venues/{id}/payments`** (misma fuente de verdad que el balance diario): suma de `totalCOP`, `cashCOP`, `transferCOP` de los pagos con `date` dentro del rango. Es dinero **realmente cobrado**, no precios teóricos. | KPIs de ingresos = pagos reales |
| RN-05 | El **comparativo** se hace contra el período inmediatamente anterior de **igual duración** (ej. "este mes" vs. "mes pasado"; rango de 7 días vs. los 7 días previos). Se muestra el delta absoluto y porcentual. | Flecha ▲/▼ + % en cada KPI |
