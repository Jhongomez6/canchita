...
Sistema que permite a los jugadores de un partido cerrado votar por el "MVP" (Jugador Más Valioso). El reconocimiento se procesa en tiempo real y, luego de 2 horas desde el cierre del partido, los votos se bloquean.
...
| 3 | Tiempo Límite (2h) | Existe un candado de seguridad: la votación cuenta con una ventana máxima de **2 horas** contadas a partir de la captura del timestamp `closedAt` (generado al mandar a cerrar el partido). Pasado el tiempo, los botones se inhabilitan en cliente y la mutación falla en Backend. Implementado como `hoursSinceClosed > 2` en `lib/mvp.ts`. |
...
*   `voteForMVP(matchId, voterUid, targetId)`: Envuelve el voto en un `runTransaction(db)` de Firebase para prevenir carreras asíncronas.
    *   Lee la fecha actual `now()` contra la fecha `closedAt` registrada. Lanza un "Error: Periodo terminado" si la diferencia es mayor a 2 horas.
