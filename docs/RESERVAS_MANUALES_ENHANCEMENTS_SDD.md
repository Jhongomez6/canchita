...
| A11 | **Tap en card body solo activo para reservas canceladas** | `clickable` en `AdminBlockCard` ahora es `true` únicamente cuando `isCancelled(block)`. Para reservas activas, el tap en el cuerpo de la card no hace nada — todas las acciones van por los botones del footer (avanzar, editar, cancelar). Para reservas canceladas, el tap abre el `DeleteBlockedSlotSheet` (hard delete o eliminación de instancia recurrente), que es la única acción relevante sobre un registro ya cancelado. |
