...
import { Heart, BarChart2, HelpCircle, History, ChevronRight } from "lucide-react";
...
export default function StatsCard({ stats, mvpAwards, historyHref }: StatsCardProps) {
    ...
    return (
        <div id="statistics" className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 mb-6">
            ...
            {/* Acceso al historial completo de partidos */}
            {historyHref && (
                <Link
                    href={historyHref}
                    className="mt-5 pt-4 border-t border-slate-100 flex items-center gap-3 -mx-1 px-1 py-1 rounded-xl active:opacity-60 transition-opacity"
                >
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500">Todos tus partidos y resultados</p>
                    </div>
