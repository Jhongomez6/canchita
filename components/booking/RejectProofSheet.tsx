...
import { rejectBookingRequest } from "@/lib/bookings";
...
interface RejectProofSheetProps {
    ...
    onRejected?: (newStatus: "cancelled") => void;
}
...
export default function RejectProofSheet({ open, onClose, booking, onRejected }: RejectProofSheetProps) {
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            const res = await rejectBookingRequest(booking.id, trimmed);
            await logBookingProofRejected({
                venueId: booking.venueId,
                bookingId: booking.id,
                attemptNumber: (booking.paymentProofHistory?.length ?? 0) + 1,
            });
            toast.success("Solicitud rechazada · jugador avisado");
            onRejected?.(res.status);
            onClose();
        } catch (err) {
            handleError(err, "No pudimos rechazar la solicitud");
        } finally {
            setSubmitting(false);
        }
    };
                    ...
                    >
                        <div className="p-5 pb-[calc(env(safe-area-inset-bottom,0px)+24px)] md:pb-5">
                            ...
                            <p className="text-sm text-slate-500 mb-4">
                                La solicitud quedará cancelada y el jugador recibirá el motivo.
                                Si quiere, podrá crear una nueva solicitud desde cero.
                            </p>
